...
import { ambushText, cardCost, cardOf, evolutionBoosts, findNode, legalPlays, other, reactionOptions, SLOT_LABEL, STANCES } from '../../engine';
...
import { FACTION_META, PLAYER_COLORS, STANCE_META } from '../meta';
        ...
        {over ? (
          ...
          <section className="shrink-0 rounded-xl border border-accent/60 bg-panel p-2 lg:grid lg:grid-cols-[minmax(0,1fr)_290px] lg:gap-3" aria-label="Your hand">
              ...
              <div className="mb-2 rounded-lg bg-black/30 p-2 text-xs">
                ...
                {cycleMode ? (
                  <div className="mt-1.5 flex gap-2">
                    <button className="rounded-md bg-sky-700 px-3 py-1.5 font-semibold" onClick={() => send({ type: 'CYCLE', player: me, uid: selected, mode: 'vent' })}>
                      Discard → vent {state.config.cycle.ventAmount}
                    </button>
                    <button className="rounded-md bg-sky-700 px-3 py-1.5 font-semibold" onClick={() => send({ type: 'CYCLE', player: me, uid: selected, mode: 'draw' })}>
                      Discard → draw {state.config.cycle.drawAmount}
                    </button>
                  </div>
                ) : whyNot(selected) ? (
                  <div className="mt-1 text-amber-300">{whyNot(selected)}</div>
                ) : selDef.type === 'graft' ? (
                  <div className="mt-1 flex flex-wrap items-center gap-2 text-accent">
                    Tap a glowing slot on your Specimen.
                    {replaceSlots.length > 0 && <span className="text-amber-300">Occupied glowing slots replace that graft (+{state.config.replace.extraCost} Energy).</span>}
                    {canFaceDown && (
                      <label
                      ...
                      >
                        <input type="checkbox" checked={faceDown} onChange={(e) => setFaceDown(e.target.checked)} /> Face-down: asleep, wake later for Ambush ({ambushText(state.config, mine.faction)})
                      </label>
                    )}
                  </div>
                ) : selDef.effect.target === 'enemySlot' ? (
                  <div className="mt-1 text-accent">Tap a glowing enemy graft to target it.</div>
                ) : (
                  <button className="mt-1.5 rounded-md bg-accent px-3 py-1.5 font-bold text-black" onClick={() => send({ type: 'PLAY_CARD', player: me, uid: selected })}>
                    Play {selDef.name}
                  </button>
                )}
              </div>
            ...
            <div className="flex flex-wrap gap-2">
              {confirmingPass ? (
                <div className="flex flex-2 flex-wrap items-center gap-2 rounded-lg bg-amber-900/40 px-2 py-1 text-xs">
                  <span className="min-w-0 flex-1">You can still play a card.</span>
                  <button onClick={() => send({ type: 'PASS', player: me })} className="rounded-md bg-amber-500 px-2 py-1 font-bold text-black">
                    Pass anyway
                  </button>
                  <button onClick={() => setConfirmingPass(false)} className="rounded-md bg-panel2 px-2 py-1 font-semibold">
                    Keep playing
                  </button>
                </div>
              ) : (
...
function Intro({ state, onGo, onExit }: { state: GameState; onGo: () => void; onExit: () => void }) {
  return (
    <div className="mx-auto flex min-h-dvh max-w-3xl flex-col gap-3 p-3">
      ...
      <p className="text-xs text-ink2">Both players see both skill-tree loadouts. Seed {state.seed}.</p>
      <div className="grid gap-3 sm:grid-cols-2">
        {state.players.map((p) => (
          <section key={p.id} className="rounded-xl border border-line bg-panel p-3">
            ...
            <div className="mt-2 text-[10px] font-bold uppercase tracking-wide text-mute">Skill tree</div>
