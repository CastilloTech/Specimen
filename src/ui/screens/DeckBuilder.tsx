...
import { CARDS, defaultConfig, FACTIONS, STARTER_DECKS, treeRows, validateDeck } from '../../engine';
import type { CardType, Faction } from '../../engine';
...
import { loadDecks, loadLoadouts, saveDecks, saveLoadout } from '../storage';

const D = defaultConfig.deck;

function DeckTab() {
  const [faction, setFaction] = useState<Faction>('predator');
  const [counts, setCounts] = useState<Record<string, number>>(() => tally(STARTER_DECKS.predator));
  ...
  const errors = validateDeck(faction, deck);
  const total = deck.length;
  const facCount = deck.filter((id) => CARDS.find((c) => c.id === id)?.faction === faction).length;
  const techCount = deck.filter((id) => CARDS.find((c) => c.id === id)?.faction === 'tech').length;
  const pool = CARDS.filter((c) => (c.faction === faction || c.faction === 'tech') && (filter === 'all' || c.type === filter));

  const change = (id: string, delta: number) => setCounts((c) => ({ ...c, [id]: Math.max(0, (c[id] ?? 0) + delta) }));
  const switchFaction = (f: Faction) => {
    setFaction(f);
    setCounts(tally(STARTER_DECKS[f]));
    ...
    <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_300px]">
      ...
      <aside className="h-fit rounded-xl border border-line bg-panel p-3 lg:sticky lg:top-2">
        ...
        <ul className="mt-1 space-y-0.5 text-xs">
          ...
          <li className={facCount >= D.minFaction ? 'text-emerald-300' : 'text-amber-300'}>
            {FACTION_META[faction].name} cards {facCount} (min {D.minFaction})
          </li>
        ...
        <div className="mt-2 flex gap-2">
          <button disabled={errors.length > 0 || !name.trim()} onClick={() => persist([...saved, { id: `d${Date.now()}`, name: name.trim(), faction, cards: deck }])} className="flex-1 rounded-lg bg-accent px-3 py-2 text-sm font-bold text-black disabled:opacity-40">
            Save deck
          </button>
          <button onClick={() => setCounts(tally(STARTER_DECKS[faction]))} className="rounded-lg bg-panel2 px-3 py-2 text-sm font-semibold">
            Starter
          </button>
        </div>
...
function LoadoutTab() {
  ...
  const value = loadouts[faction] ?? treeRows(faction).map((r) => r.nodes[0].id);
  const update = (l: string[]) => {
    setLoadouts((cur) => ({ ...cur, [faction]: l }));
    saveLoadout(faction, l);
  };
  return (
    <div className="max-w-3xl">
      <div className="mb-3 flex gap-2">
        {FACTIONS.map((f) => (
          <button key={f} onClick={() => setFaction(f)} aria-pressed={faction === f} className={`rounded-lg border px-3 py-1.5 text-sm font-bold ${faction === f ? 'bg-black/30' : 'border-line'}`} style={{ color: FACTION_META[f].color, borderColor: faction === f ? FACTION_META[f].color : undefined }}>
            {FACTION_META[f].name}
          </button>
        ))}
      </div>
      <p className="mb-2 text-xs text-ink2">Pick exactly one node in each row. All nodes are unlocked. Your choice is saved automatically and used as the default in match setup.</p>
      <LoadoutPicker faction={faction} value={value} onChange={update} />
    </div>
  );
}
  ...
  return (
    <div className="mx-auto min-h-dvh max-w-6xl p-3">
      <div className="mb-3 flex items-center gap-2">
        ...
        <h1 className="text-xl font-bold">Decks &amp; skill trees</h1>
