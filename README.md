...
Numbers below are from the Fourteenth pass's sample (7,000 matches). Everything here predates full 30,000-match tuning, and several bullets from earlier passes that named specific cards, nodes, or matchups no longer apply (the Fourteenth pass in particular closed most of what this list used to say) — removed rather than left stale.

- **Round 2 is still an Energy plateau**: the curve is 2, 2, 3, 4, 5, 6. Smoothing it (shift the whole curve up one round) broke Predator vs. Parasite badly as a side effect two sessions ago and was reverted; that matchup only just stabilized this pass, so it was deliberately not retried here. The next attempt should be its own isolated `--config` sweep, checked against all three Build matchups before it goes anywhere near the real config.
- **Parasite vs. Aegis (59.7%) and Predator/Parasite vs. Miasma (44–48%)** are the widest gaps left after this pass, but none of them are in the 33–45% "structural outlier" territory everything above started in (Bastion was 35.3% vs. Parasite, Aegis was 38.7% vs. Hollow, before the Fourteenth pass) — these read as normal tuning noise at a 7,000-match sample, not a mistake to chase.
- **The 12 Build x World Faction archetypes are still not perfectly even relative to each other** — expected at this stage (`npm run sim` is run "moderate," not exhaustive), and a full 30,000-match-per-cell pass is the natural follow-up now that both axes are close to centered individually rather than fighting a structural gap.
- **Snowballing (70.6%, was 74–76%)**: improved by a stronger catch-up draw this pass, but a lead after round 3 still mostly decides the game. A second, more targeted lever (a bigger HP-gap threshold cut, or a Strain/Energy-based catch-up alongside the card-draw one) is the next thing to try if it still matters after a real playtest.
