...
Numbers below are from the Fifteenth pass's sample (8,000 matches for `sim`, 6,000 for `audit`). Everything here predates full 30,000-match tuning, and several bullets from earlier passes no longer apply (the Fourteenth and Fifteenth passes between them closed most of what this list used to say) — removed rather than left stale.

- **Parasite vs. Aegis (56.5%) and Parasite vs. Miasma (~43%) are still open, and look structural rather than power-level.** Nerfing Parasite's Hive Host evolution brought overall Build balance to near-perfect but barely moved either gap; buffing Miasma's signature (raw attack) moved Parasite vs. Miasma only slightly. The working theory (not yet acted on): Aegis's kit (integrity-heal, Ward Pact) and Miasma's kit (Numb/Fever) both target things Parasite's own play pattern barely touches — Protocols, high-cost grafts, forced rejections — so the fix likely needs an Aegis- or Miasma-side card that specifically punishes Parasite's strain-drain/heal-on-reject pattern, not another stat pass on either Build or World Faction. Neither is in the 33–45% "structural outlier" territory the Fourteenth pass started from, so this is a real but bounded gap, not an emergency.
- **Corrosion vs. Aegis (44.6%) did not move across any change made this pass**, despite Hollow, Miasma, and both Parasite matchups all shifting from the same sim runs — it reads as a genuine standing asymmetry between those two World Factions specifically, worth its own investigation rather than being swept up by broader changes.
- **The 12 Build x World Faction archetypes are close but not perfectly even** — most cells now sit inside 45–55%, with Parasite/Aegis, Parasite/Miasma, Parasite/Corrosion (44.4%), and Predator/Corrosion (53.8%) the remaining outliers. A full 30,000-match-per-cell pass is the natural follow-up now that both axes are close to centered individually.
